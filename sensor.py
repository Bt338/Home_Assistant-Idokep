...
#   )
    ...
    if create_observations is True:
        ...
        for idx in range(0, len(OBSERVATION_SENSOR_TYPES)):
            new_entities.append(
                ObservationSensor(
                    hass_data,
                    observation_basename,
                    OBSERVATION_SENSOR_TYPES[idx].key,
                    OBSERVATION_SENSOR_TYPES[idx],
                )
            )
...
#           CONF_FORCAST_DAYS, 
#           config_entry.data.get(CONF_FORCAST_DAYS, 0),
#       )
#       #_LOGGER.warning("[%s]",FORECAST_SENSOR_TYPES)
#
#       for day in range(0, forecast_days):
#           for idx in range(0,len(FORECAST_SENSOR_TYPES)-1):
#               #_LOGGER.warning("[%s]",idx)
#               #_LOGGER.warning("[%s]: [%s]",(idx,FORECAST_SENSOR_TYPES))
#               new_entities.append(
#                   ForecastSensor(
#                       hass_data,
#                       forecast_basename,
#                       day,
#                       FORECAST_SENSOR_TYPES[idx].key,
#                       FORECAST_SENSOR_TYPES[idx],
#                   )
#               )
#   
...
class ObservationSensor(SensorBase):
    ...
    def native_value(self):
        ...
        return self.collector.locations_data["current"][self.entity_description.key]
...
class ForecastSensor(SensorBase):
    ...
    def extra_state_attributes(self):
        ...
        if self.day < len(self.collector.daily_forecasts_data):
            tzinfo = pytz.timezone(self.collector.locations_data["data"]["timezone"])
        ...
        if self.day < len(self.collector.daily_forecasts_data):
            if self.device_class == SensorDeviceClass.TIMESTAMP:
                tzinfo = pytz.timezone(
                    self.collector.locations_data["timezone"]
                )
                return self.collector.daily_forecasts_data[self.day][self.sensor_name]
            """
            if self.sensor_name == "uv_forecast":
                if (self.collector.daily_forecasts_data[self.day]["uv_category"] is None):
                    return None
                if (self.collector.daily_forecasts_data[self.day]["uv_start_time"] is None):
                    return (
                        f"Sun protection not required, UV Index predicted to reach "
                        f'{self.collector.daily_forecasts_data[self.day]["uv_max_index"]} '
                        f'[{self.collector.daily_forecasts_data[self.day]["uv_category"].replace("veryhigh", "very high").title()}]'
                    )
                else:
                    utc = pytz.utc
                    local = timezone(self.collector.locations_data["timezone"])
                    start_time = utc.localize(datetime.strptime(self.collector.daily_forecasts_data[self.day]["uv_start_time"], "%Y-%m-%dT%H:%M:%SZ")).astimezone(local)
                    end_time = utc.localize(datetime.strptime(self.collector.daily_forecasts_data[self.day]["uv_end_time"], "%Y-%m-%dT%H:%M:%SZ")).astimezone(local)
                    return (
            ...
            new_state = self.collector.daily_forecasts_data[self.day][self.sensor_name]
    ...
    def name(self):
        """Return the name of the sensor."""
        return f"{self.location_name} {self.entity_description.name.replace('_', ' ').title()} {self.day}"
...
class NowLaterSensor(SensorBase):
    ...
    def state(self):
        """Return the state of the sensor."""
        self.current_state = self.collector.daily_forecasts_data[0][
            self.sensor_name
        ]
        return self.current_state
